...
def read_anc_daily(hdf_files):
      ...
      for i in range(len(hdf_files)):  # loop over hdf files in day
          df_i = []
          print(hdf_files[i]) #- use to output if there is troube reading
...
def spec_plot(spec, name, units ,filename):
       ...
       plt.title(name + ' : ' + str(spec.index[0].date()) + ': N =' + str(len(spec)))
       plt.plot(wl, np.zeros(len(np.array(spec).T)),linestyle='dashed',color='k')
       plt.plot(wl, spec.T)
       plt.xlabel('Wavelength [nm]')
       plt.ylabel(units)
       #plt.tilt('N =' + str(len(spec)))
       ##plt.ylim(0,0.005)
...
if __name__ == '__main__':
    ...
    ###############################################
    hdf_files_list = []
    for i in range(17,len(jday_dir)):
        
        hdf_files_list =[]
    
        for k in range(i,i+1): # up to jday 351
      ...
      #  spec_plot(qaa_adg,'adg (QAA)','[m$^{-1}$]')
        ...
        min_vis = np.min(Rrs_M02.iloc[:,91:105],axis=1).values # min value on 650-700 nm >0
        Rrs = Rrs[min_vis > 0]
        Rrs_M02 = Rrs_M02[min_vis > 0]
        Rrs_unc = Rrs_unc[min_vis > 0]
        anc = anc[min_vis > 0]
        ...
        wind =  np.array(anc['WINDSPEED'])
         ...
         #  spec_plot(Rrs,'Rrs','[sr$^{-1}$]')
        figname  =   '/mnt/d/AMT31/Optics_all/Data/HSAS/HSAS_data/Processed/RrsSb/Figs/M02_Rrs_' + str(date[0]) + ".png"
        spec_plot(Rrs_M02,'Rrs_M02','[sr$^{-1}$]', figname)
        # spec_plot(Rrs_unc,'Rrs_unc: absolute', '[sr$^{-1}$]')
       ...
       # filename = '/mnt/d/AMT31/Optics_all/Data/HSAS/HSAS_data/Processed/RrsSb/' + "AMT31_Rrs_" + str(date[0]) + "_" + str(date[-1]) + "_QC.sb"
        filename = '/mnt/d/AMT31/Optics_all/Data/HSAS/HSAS_data/Processed/RrsSb/' + "AMT31_Rrs_" + str(date[0]) + "_QC.sb"
