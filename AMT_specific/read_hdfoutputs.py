...
def read_hdf_L2(file, grp, var):
    ' function to read hdf file and open in pandas dataframe format - individual fiule'
...
def read_hdf_L2_totaldeployment(file, grp, var):
...
def spec_plot(spec, name, units):
       ...
       plt.title(name + ' : ' + str(spec.index[0].date()))
...
if __name__ == '__main__':
    ...
    ###############################################
    hdf_files_list = []
    for k in range(len(jday_dir)):
        hdf_files_list.append(glob.glob(jday_dir[k] + '/L2/*hdf*'))
    
        
    index = -4 # choose index for jday. i=0 is jday 342
    
    ################################################################
    # 1. Deriving ancillary data  in daily pandas dataframes
    ################################################################
    
    hdf_files = hdf_files_list[index] # use 0th file to test functions
    f = h5py.File(hdf_files[index], 'r')
    
    anc_var = list(f['ANCILLARY'].keys()) # list anc lies
    anc = anc_to_df(hdf_files)
    ...
    ################################################################
    # 2. Deriving ancillary data in daily pandas dataframes
    ################################################################
    ...
    # Reflectance
    Rrs = read_hdf_L2_daily(hdf_files, 'REFLECTANCE', 'Rrs_HYPER') # Rrs that has no BRDF correction
    Rrs_M02 = read_hdf_L2_daily(hdf_files, 'REFLECTANCE', 'Rrs_HYPER_M02') # Rrs that has Morel BRDF
    nLw_M02 = read_hdf_L2_daily(hdf_files, 'REFLECTANCE', 'nLw_HYPER_M02') # nLw that has Morel BRDF
    Rrs_unc = read_hdf_L2_daily(hdf_files, 'REFLECTANCE', 'Rrs_HYPER_unc') # Rrs that has
    ...
    # check Rrs in % units
    # Rrs_unc = read_hdf_L2_daily(hdf_files, 'REFLECTANCE', 'Rrs_HYPER_unc') # Rrs that has
    #Rrs_unc = 100*(Rrs_unc/Rrs)
    #spec_plot(Rrs_unc.iloc[:,0:70], 'Rrs_unc: [%]','[%]')
    
    # 
    bincount = read_hdf_L2_daily(hdf_files, 'REFLECTANCE', 'Ensemble_N') #rrs bin count
    bincount = bincount['N'].values.astype(int)
  
    
    yyyymmdd = str(Rrs_M02.index[0].date())[0:4] + str(Rrs_M02.index[0].date())[5:7] + str(Rrs_M02.index[0].date())[8:10]
    start_time = str(Rrs_M02.index[0].time()) + '[GMT]' 
    end_time = str(Rrs_M02.index[0].time()) + '[GMT]' 
    min_lat = str(min(lat))[0:7]
    max_lat = str(max(lat))[0:7]
    min_lon = str(min(lon))[0:7] 
    max_lon = str(max(lon))[0:7]
    station = -9999*np.ones(len(yyyymmdd)).astype(int)
    time.astype(str)
    ...
    "/cruise=": "AMT_31",
    "/station=": "NA",
    "/data_file_name=": "AMT31_Rrs_" + yyyymmdd,
    ...
    filename = '/mnt/d/AMT31/Optics_all/Data/HSAS/HSAS_data/Processed/DailyRrsSb/' + "AMT31_Rrs_" + yyyymmdd + '.sb'
